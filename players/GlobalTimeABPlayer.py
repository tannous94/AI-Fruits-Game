...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        if self.flag:
            num_turns = math.ceil(self.available_cells(curr_state) / 2)
            #board_total_size = len(curr_state.board) * len(curr_state.board[0])
            self.time_frame = self.global_time / num_turns #(self.global_time * num_turns) / board_total_size
            self.flag = False

        score, best_move, num_of_leaves = self.alphabeta.search(curr_state, dep, True)

        last_iter_time = time.time() - start
        new_time_limit = copy.deepcopy(self.time_frame) - last_iter_time
        #next_iter_time = self.iter_time_limit(num_of_leaves, last_iter_time, dep)
        #curr_time = time.time() - start

        while 0 < new_time_limit:
            dep = dep + 1
            iter_start = time.time()
            score, best_move, num_of_leaves = self.alphabeta.search(curr_state, dep, True)
            last_iter_time = time.time() - iter_start
            new_time_limit = new_time_limit - last_iter_time
            #next_iter_time = self.iter_time_limit(num_of_leaves, last_iter_time, dep)
            #curr_time = time.time() - start
        ...
        self.global_time = self.global_time - self.time_frame
