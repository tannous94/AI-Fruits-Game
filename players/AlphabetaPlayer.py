...
class Player(AbstractPlayer):
    ...
    def make_move(self, time_limit, players_score):
        ...
        """
        # TODO: not complete and needs editing (time and players scores and minimax algorithm)
    ...
    def heuristic(self, state):
        ...
        if ratio < 0.4:
            if bool(fruits):
                heuristic_result = ((2 * available_moves_count) - (2 * rival_available_moves_count)
                                    + len(possible_blocking_moves) + len(succ_available_moves) + 0.3 *
                                    state.players_scores[0]
                ...
                heuristic_result = ((2 * available_moves_count) - (2 * rival_available_moves_count)
                                    + len(possible_blocking_moves) + len(succ_available_moves) + 0.1 *
                                    state.players_scores[0])
        else:
            if bool(fruits):
                heuristic_result = ((3 * available_moves_count) - (1 * rival_available_moves_count)
                                    + len(possible_blocking_moves) + (2 * len(succ_available_moves)) + 0.1 *
                                    state.players_scores[0]
